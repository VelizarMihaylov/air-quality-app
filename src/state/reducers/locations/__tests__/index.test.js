...
describe('city', () => {
  ...
  it('should return the default state when called without matching action type', () => {
    expect(locations(initialState, { type: 'INVALID_ACTION' })).toEqual([])
  })
  it('should update the store with locations when called with ADD_LOCATIONS action and payload', () => {
    ...
    expect(locations(initialState, { type: 'ADD_LOCATIONS', payload: locationsMock })).toEqual(locationsMock)
  })
  it('should remove a location from the store when called with REMOVE_LOCATION action and payload', () => {
    ...
    expect(locations(initialState, { type: 'REMOVE_LOCATION', payload: 'Manchester Piccadilly' })).toEqual([
      {
        city: 'Manchester',
        location: 'Salford Eccles'
      }
